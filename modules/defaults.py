...
class DefaultTrainer(TrainerBase):
    ...
    def parse_ckpt(self, ckpt_dir):
        ...
        try:
            ...
            print('try to load ckpt from {} using pickle(latin1 encoding)'.format(ckpt_dir))   
            import pickle
    ...
    def modified_checkpointer(self, resume):
        ...
        checkpoint = DetectionCheckpointer(checkp, save_dir=self.cfg.OUTPUT_DIR).resume_or_load(
    ...
    def resume_or_load(self, resume=True):
        ...
        if resume and self.checkpointer.has_checkpoint():
            try:
                print("loading _train from checkpoint")
                self._trainer.load_state_dict(checkpoint['trainer'].pop('_trainer'))

                print("loading scheduler from schekpoint")
                self.scheduler.load_state_dict(checkpoint['trainer']['hooks'].pop('LRScheduler'))
            except:
                print('no scheduler to load from checkpoint: {}'.format(self.cfg.MODEL.WEIGHTS))
                print('no _trainer to load from checkpoint: {}'.format(self.cfg.MODEL.WEIGHTS))
            ...
            print('#'*10, 'start_iter', self.start_iter)
...
"""
    ...
    def test(cls, cfg, model, evaluators=None):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TEST):
            ...
            results_i = inference_on_dataset(model, data_loader, evaluator, save=True)
...
def inference_on_dataset(
    ...
    if save:
        os.makedirs(os.path.join(evaluator._output_dir, 'vis'), exist_ok=True)
        os.makedirs(os.path.join(evaluator._output_dir, 'csv'), exist_ok=True)
        os.makedirs(os.path.join(evaluator._output_dir, 'semseg'), exist_ok=True)
    ...
    with ExitStack() as stack:
        ...
        for idx, inputs in enumerate(data_loader):
            ...
            if save:
                ...
                df.to_csv(os.path.join(evaluator._output_dir, 'csv', os.path.basename(inputs[0]['file_name']).split('.')[0]+'.csv'), index=False, header=False)

                sem_seg = outputs[0]['sem_seg'].cpu()
                torch.save(sem_seg, os.path.join(evaluator._output_dir, 'semseg', os.path.basename(inputs[0]['file_name']).split('.')[0]+'.pt'))
