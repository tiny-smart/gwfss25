...
class AMPTrainer(SimpleTrainer):
    ...
    def run_step(self):
        ...
        if self.zero_grad_before_forward:
            self.optimizer.zero_grad()
        # with autocast(dtype=self.precision):  # torch 1.10.0+
        with autocast():  # torch 1.9.0
            loss_dict = self.model(data)
            if isinstance(loss_dict, torch.Tensor):
                losses = loss_dict
                loss_dict = {"total_loss": loss_dict}
...
class SimpleTrainerSSL(TrainerBase):
    ...
    @staticmethod
    def _generate_roi(img_size, roi_size, align_k=4):
        max_pos = (img_size - roi_size) // align_k
        rand_x = random.randint(0, max_pos)
        rand_y = random.randint(0, max_pos)
        x = align_k * rand_x
        y = align_k * rand_y
...
class AMPTrainerSSL(SimpleTrainerSSL):
    ...
    def run_step(self):
        ...
        with autocast():    # low torch version
            loss_dict = self.model(data, branch='supervised')
            if isinstance(loss_dict, torch.Tensor):
                losses = loss_dict
                loss_dict = {"total_loss": loss_dict}
